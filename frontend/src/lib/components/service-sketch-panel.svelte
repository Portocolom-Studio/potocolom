<script lang="ts">
	...
	let {
		...
		mode: 'edit_image' | 'image_to_text' | 'realtime_canvas';
	} = $props();
		...
		{#if mode === 'realtime_canvas'}
			<div class="grid min-h-[32rem] flex-1 gap-4 lg:grid-cols-2">
				<Card.Root class="flex min-h-0 flex-col">
					...
					<Card.Content class="grid min-h-0 flex-1 grid-rows-[1fr_auto] gap-3">
						<button
							type="button"
							disabled
							class="border-border bg-muted/20 text-muted-foreground grid min-h-72 cursor-not-allowed place-items-center rounded-lg border border-dashed px-6 text-center text-sm"
						>
							{t('app.realtime_canvas.draw_surface')}
						</button>
						<button
							type="button"
							disabled
							class="border-border text-muted-foreground cursor-not-allowed rounded-lg border border-dashed px-4 py-3 text-sm"
						>
							{t('app.realtime_canvas.drop_image')}
						</button>
					</Card.Content>
				</Card.Root>
				<Card.Root class="flex min-h-0 flex-col">
				...
				<Card.Root>
					<Card.Header>
						<Card.Title class="text-base">
							{mode === 'edit_image'
								? t('app.edit_image.controls')
								: t('app.image_to_text.controls')}
						</Card.Title>
					</Card.Header>
