...
|---|---|---|
| **Database connection pooling.** `backend/app/db.py` keeps a bounded pool (`pool_size=5, max_overflow=10`), and the tests drive requests and WebSockets on one event loop. | Fifteen connections per API process is the ceiling a managed PostgreSQL sees. | Tune the pool per profile once more than one replica runs against a shared instance. |
