...
**SES feedback.** Shipped. `POST /api/v1/mail/feedback` accepts SNS bounce and complaint notifications, verifies the signature against the regional SNS key, and honours only the topic named by `SES_FEEDBACK_TOPIC_ARN`. The signature says an AWS customer sent the message, the topic says it was ours, and the topic's access policy, which admits only SES with a `SourceArn` condition, is what keeps the topic worth checking. Setup is step 8 of [aws-setup.md](aws-setup.md).
