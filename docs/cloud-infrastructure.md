...
- Dispatch uploads land under `dispatch/{user_id}/` and are promoted into the durable `{user_id}/` library prefix on commit. An S3 lifecycle rule expires the `dispatch/` prefix after 24 hours as a backstop when a presigned PUT is replayed after cleanup (issue #278).
