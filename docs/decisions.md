...
Measured on the reference development card (Radeon RX 7600 XT, gfx1102) on 2026-08-02, using the worker's own calibration path so the number means what the scheduler means by it: `vega-rt` at full residency returns a single-frame p95 of 285.9 ms and 278.8 ms across two runs, which earns **one** slot, not two. Raising the configured cap from 2 to 8 changed nothing, because p95 binds and the cap does not. Two consequences follow. The cost range in this entry should be read at its pessimistic end for hardware of this class, since `ceil(1000 / 1)` is 1000 processes rather than 500. And the next density boundary is closer than the published headroom suggests: 285.9 ms needs only a 13 percent reduction to reach the 250 ms two-slot threshold and halve cost per active session, against the 2.3x reduction that four slots would require. This is one consumer card and not the fleet card, so the absolute figure does not transfer; the structure does.

The consequence is that a worker-side pipeline change is the highest-value cost lever, ahead of relay, gateway, and transport work. Every density and quality claim needs measurement on the selected fleet card, so the next experiment is gated on GPU availability, currently unavailable for about 70 hours, rather than on design.
