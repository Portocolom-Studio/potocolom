...
So more than one slot is advertised only when there is exactly one realtime model to serve and that model is the one calibration measured. Everything else advertises one, where no two sessions run steadily side by side and the bar depends on the model the single session chose rather than on a model nobody timed.
