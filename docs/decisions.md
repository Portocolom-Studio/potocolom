...
## Realtime capacity is promised only for models that were measured

A worker calibrates one realtime model at boot and `hello` carries one `realtime_slots`, which admission then checks whatever model a session asks for. The slot formula `min(cap, floor(500 / p95))` is therefore sound only for the model that was measured: sessions serialise on the GPU lock, so two slots earned by a model at 240 ms become 560 ms per cycle as soon as one of those sessions runs a model at 280 ms, and the bar the number promised is missed by a model nobody timed. Nothing breached it while one model was realtime, and #274 made two of them realtime.

So more than one slot is advertised only when there is exactly one realtime model to serve and the capacity is computed from that model's own measured frame time, whether calibration timed it at boot or a session's observed frames did. Anything else that can serve a realtime session at all advertises one, where no two sessions run steadily side by side and the bar depends on the model the single session chose rather than on a model nobody timed. Zero stays zero: no realtime capability on the wire, no configured slots, or a calibration that already refused the bar all advertise nothing, and this rule does not turn any of those into one.

Deriving the number from the measurement rather than requiring one to exist is the part that took three attempts to get right. A model can be timed without calibration having run, because observed session frames feed the same store, and the engine falls back to the configured limit when calibration never happened at all. Asking only whether a timing existed therefore advertised the configured capacity for a model measured at 400 ms, which earns one slot and not two.

The count comes from the manifests, not from the wire and not from what is resident, because both of those move without a reconnect: a rung recomputed from current free VRAM, or an eviction during a queued job, changes which model is full without changing what the worker can be asked to serve, and it is the asking that the number promises against. Two earlier attempts at this got it wrong in exactly that way, first capping the bound passed to calibration at boot, which a reconnect skips, then counting the wire, which a rung change invalidates between hellos. A `benchmark_only` model counts as well: the studio never offers one, but the realtime endpoint resolves models through `available()`, and `sd-turbo` ships as `benchmark_only` with `realtime`, so a session on it is admissible and would serialise unmeasured frames. A worker whose only realtime model is one warmup refuses to warm therefore advertises one slot rather than its configured capacity, because nothing was measured at all.

One slot is not a claim that two frames never overlap. Releasing a slot happens before the departing worker's runner has finished its last frame, and cancelling that runner deliberately holds the GPU lock until in-flight work completes, so a newly admitted session's first frame can queue behind most of an old one. That is a bounded overlap at teardown, not two sessions serialising for the length of a session, which is what the slot number is about.

On the reference card this changes nothing today, since `sdxl-turbo` at 278 ms and `vega-rt` at 337 ms both earn a single slot; it removes a promise that would have become false the first time a realtime model measured under 250 ms.

Rejected alternatives: calibrating every realtime model at boot, which is the version that keeps full capacity and costs a cold load each, measured at 15.4 s for `sdxl-turbo` alone, before a worker can register; advertising capacity from the slowest model, which needs those same measurements to know which is slowest; per-model slots on the wire, which is the honest shape and a change to `hello`, to admission, and to slot accounting that assumes one counter, worth doing when a fleet serves enough realtime models for the lost capacity to matter; and admitting sessions only for the calibrated model, which would make the picker's other entries unusable and turn a capacity question into a feature removal.
