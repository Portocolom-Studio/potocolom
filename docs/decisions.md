...
Replacing the decoder with `madebyollin/taesdxl`, 2.45M parameters against roughly 84M and MIT licensed, takes the decode from 123.7 ms to 8.3 ms. End-to-end `frame()` falls from about 274 ms to 184.9 ms and calibration moves from one slot to three, so cost per active session drops roughly threefold. Decoded output is visually near-identical: composition, colour and brushwork hold, with slightly softer fine texture, which is an easy trade for a frame that is replaced two to four times a second.

Replacing the encoder as well is faster still and must not be done. Swapping the whole autoencoder brings a frame to 94.0 ms and five slots, but the image changes subject rather than losing detail: in a fixed-seed comparison the lighthouse the prompt asked for was replaced by a flat-topped rock, at a mean per-channel difference of 22.4/255, which is larger than the difference from adding a whole new clause to the prompt. The cause is structural. The encoder fixes the latent the UNet denoises from, and at strength 0.7 with roughly one effective step there is almost no denoising left to correct an approximate starting point, so the encoder's error survives into the output. A realtime preview is what the user draws against, so a preview that does not show what is being made is worse than a slower one.
