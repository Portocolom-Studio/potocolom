...
What a gated login hands back is a challenge, and a challenge is not a session. It carries a ten minute life, lives in its own cookie, and authorizes nothing: the right code exchanges it for a session, and there is nothing else it can do. Ten wrong answers end it, and the budget is meant to belong to the account rather than to the challenge, because starting another challenge costs nothing to anyone who already has the password. Beginning one spends the account's outstanding challenges and carries their used attempts forward, which holds for logins taken one at a time and does not hold for logins that overlap: two that begin together both seed a fresh budget, and the same window lets one challenge be answered twice. Issue #421 tracks closing it. The intent is recorded here because it is the intent, not because the current code reaches it.
