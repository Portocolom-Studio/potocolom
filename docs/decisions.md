...
So a worker offering more than one realtime model advertises at most one slot. One session never serialises against another, which makes the bar depend only on the model that session chose, and the calibrated number keeps its meaning as the label the picker shows. A worker offering exactly one keeps the configured bound, because then nothing is unmeasured. On the reference card this changes nothing today, since `sdxl-turbo` at 278 ms and `vega-rt` at 337 ms both earn a single slot; it removes a promise that would have become false the first time a realtime model measured under 250 ms.
