...
The address bucket never refuses, and the shared peer above is the reason. The specification names thirty per IP and does not say whether that number is a ceiling or a pace, and read as a ceiling it turns a shared peer into an outage: everybody behind one NAT, one proxy that has not been named in `FORWARDED_ALLOW_IPS`, or the loopback publish the shipped compose file uses arrives here as a single address, so thirty attempts between all of them would sign the whole installation out of `/api/v1/auth/login` ten minutes at a time. That is a denial of service the limit creates rather than one it prevents. Read as a pace it costs a bystander nothing: the address count feeds the wait, and an attacker at one peer still drops from thousands of attempts a second to a few hundred an hour once the eight second cap arrives, which is the bound the number was asking for. The account keeps a ceiling either way, because the identifier bucket is scoped to the one account it protects and a refusal there reaches nobody else.
