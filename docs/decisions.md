...
So a worker offering more than one realtime model advertises at most one slot, and a worker offering exactly one keeps the configured bound because nothing is unmeasured there. With a single slot no two sessions run steadily side by side, so the bar depends on the model the one session chose rather than on a model nobody timed, and the calibrated number keeps its meaning as the label the picker shows.

The count is taken from the wire rather than from what warmup chose to load, which matters twice. It is recomputed at every `hello`, so a reconnect that advertises a second realtime model because free VRAM changed re-applies the cap without recalibrating, and `DiffusersEngine` takes the smaller of its calibrated number and that cap rather than returning the measurement raw. And it counts every model admission can serve, including a `benchmark_only` one: the studio never offers those, but the realtime endpoint resolves models through `available()`, and `sd-turbo` ships as `benchmark_only` with `realtime`, so a session on it would serialise unmeasured frames.
