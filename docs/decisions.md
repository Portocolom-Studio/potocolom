...
Asking costs something, or it is not an ask. Ten wrong tries end it, counted before the code is looked at so a wrong one is paid for whatever happens next, and answering a login challenge with the factor gives the budget back. Without a count the caller answers the new half correctly every time and grinds the old half through six digits at leisure, and without the reset ten wrong tries by anybody would stop the owner ever moving to a new phone.
