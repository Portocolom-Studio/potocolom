...
So capacity above one session per GPU is a batching problem, and measuring it changed what that buys. Frames from concurrent sessions on the same model are collected in a short window and denoised as one batch, which is the ladder already designed above. Batching is far from free on this card: measured through the shipped adapter at 512 px, two frames as one batch cost 1.76 times one frame on `sdxl-turbo` and 1.76 on `vega-rt`, which is a saving of twelve to seventeen percent per session rather than the sublinear win a larger GPU gives. An RX 7600 XT is already compute bound at batch one, so a second sample has no idle capacity to fill.

That saving is still exactly what crosses the boundary, at the right resolution. Complete-frame p95 for `vega-rt` at its two-step floor: 512 px is 301 ms for one session and 530 ms for two, past the bar; 448 px is 255 ms for one and 422 ms for two, inside it, where serialising those two sessions would cost 510 ms and miss; 384 px is 216 ms for one, 319 ms for two and 437 ms for three, inside, where serialising three would cost 648 ms. `sdxl-turbo` at one step is 346 ms for one session and 608 ms for two, so at 512 px it serves one session per GPU whatever the scheduler does.

Concurrency is therefore a resolution decision as much as a scheduling one, and this card's ladder is one session at 512 px, two at 448 and three at 384. Peak VRAM stays at or below 5.1 GiB for `vega-rt` across all of it, so memory constrains nothing here and compute constrains everything.
