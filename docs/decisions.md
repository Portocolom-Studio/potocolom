...
## Realtime concurrency comes from batching one GPU, not from more processes on it
...
What changes is when the work starts. Both entries gate it on fleet spend, at the 500 to 1000 GPU-process scale where density is the cheapest capacity. The requirement is now a product one at a single GPU: two people drawing at once on a self-hosted box, and more than two per GPU in the cloud. Serialising cannot deliver that. Sessions share one GPU lock, so two sessions each see twice a single frame: on the reference card `sdxl-turbo` measures 278 ms for one frame, and two serialised sessions are 556 ms per cycle each, which misses the 500 ms bar and the 2 fps floor it encodes. The recorded formula agrees, admitting a second slot only at 250 ms or below, which neither shipped realtime model reaches.

So capacity above one session per GPU is a batching problem, and it is solved on this card without giving up resolution. Frames from concurrent sessions on the same model are collected in a short window and denoised as one batch, which is the ladder already designed above. Measured through the shipped adapter at 512 px, complete-frame p95: `vega-rt` at its two-step floor is 273 ms for one session and 462 ms for two, inside the bar with 38 ms to spare, where serialising those two sessions would cost 546 ms and miss; `sdxl-turbo` at one step is 301 ms and 499 ms, inside by less than a millisecond, which is not a margin to promise anything on. Three sessions need 673 ms at 512 px and 514 ms even at 448, so two is this card's honest answer and a smaller resolution buys comfort rather than a third seat.

Batching is what buys the second session, and it is not free: two frames as one batch cost 1.65 times one frame, a saving of about seventeen percent per session, because an RX 7600 XT is close to compute bound at batch one. That saving is the whole difference between 546 ms serialised and 462 ms batched. Peak allocated VRAM stays at 4.15 GiB for `vega-rt` and 7.51 GiB for `sdxl-turbo` at batch two, so memory constrains nothing in this range and compute constrains everything.

An earlier revision of this entry carried numbers 13 to 18 percent slower, from a sweep that ran without `TORCH_ROCM_AOTRITON_ENABLE_EXPERIMENTAL=1`. RDNA 3 gates its fused attention kernels behind that variable, `DiffusersEngine` sets it for `DEVICE=rocm`, and a script that builds a pipeline directly bypasses that and must set it itself, which [gpu-performance.md](gpu-performance.md) already warned about. Those numbers put two sessions past the bar at 512 px and made this look like a resolution trade, which it is not. The sweep script sets the variable at import now, before torch is imported at all.

Two sessions may share a batch only when their frames need the same graph. Model, step count, resolution and guidance must match, and so must the adapter conditioning scale, because `StableDiffusionXLAdapterPipeline` refuses a per-sample value and every session in a batch therefore shares one Structure setting; prompts and seeds may differ freely, because those are batched tensors rather than graph shape. That defines a compatibility class, and capacity is a property of a class rather than of a worker: `slots` becomes the largest N whose measured batch time for that class stays inside the bar. This is also what makes admission honest, which four attempts at a single per-worker number failed to do (issue #285): a number derived from one measurement cannot bound a session that chooses its own parameters, but a number derived per class can, because the class fixes the parameters that matter.

Capacity is therefore measured as a curve rather than a point, and `scripts/prototype-batch-sweep.py` is what measures it: it reads a manifest, batches real sketch maps through the shipped adapter, and reports the pipeline call and the whole frame separately, so any card can be characterised rather than only this one. A deployment then picks the resolution its card serves for the concurrency it wants, and the cloud reads the same curve on its own hardware.
