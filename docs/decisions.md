...
The cloud profile is to be validated locally by reproducing its topology, nginx in front of two API replicas, Redis, MinIO, Mailpit and a fake QuotaService, exercising exactly the seams the cloud uses. Today cloud-sim runs Redis, MinIO and Mailpit only; nginx, the second replica and the fake QuotaService are designed and not built (see [local-development.md](local-development.md)). The application code cannot tell nginx from an ALB or MinIO from S3, which is what the seams are for. AWS-specific control plane (Terraform, IAM, ALB behavior, CloudFront signing, SES deliverability) is validated once on the real scaled-down staging, only when the cloud launch is being prepared. Until then the infrastructure cost of development is zero. Details in [local-development.md](local-development.md).
...
First run therefore generates its secrets. The preflight script writes `deploy/compose/.env` when none exists, with `openssl rand -hex 32` for `FLEET_SECRET` and the database password, fills those keys when they are empty, refuses to overwrite a non-empty value other than the example's `change-me` database password before any database uses it, and prints what it wrote. Hex because Compose expands `$NAME` inside an unquoted value and an HTTP header carries ASCII only, both of which a generated secret has to survive.
...
An administrator reads any account and changes none of its work, with one exception: cancelling a queued or running generation. A job that is stuck, runaway or abusive holds a GPU that other people are waiting for, and the owner may not be there to stop it. The job ends with the reason "cancelled by an administrator", and a call that stops it writes a high-severity audit row carrying the owner and the job id; a repeat on a job that already ended records nothing, because nothing changed. The route takes any role, so the audit hook for administrator routes never sees it, and the route records the row itself. Starring, sharing and deleting another account's work stay refused.
...
## User content never trains models
