...
Two limits are deliberate. Behind a reverse proxy or load balancer every peer is the proxy's own private address, so a fronted deployment gains nothing here and must set the secret. And the check is about reachability, not identity: any process already on the LAN still registers, which is exactly what the entry above says the token is for.
