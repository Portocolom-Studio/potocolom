...
The account's session rows are locked in id order first. Without that, two changes racing on the same account from two different sessions deadlock: each revocation holds the row the other one is about to rotate. The five routes serialise on the account earlier in their own transactions, but not on anything they share, so a change of address and an unlink reach these statements together, and PostgreSQL breaks the cycle by killing one of them, which reaches somebody as a 500 on a password change.
...
`operator.collapse` deletes sessions after the second factor rather than before it, because of the same rotation. The command is offline, but nothing stops the API serving while somebody runs it, and confirming or removing a factor holds `auth_factors` and `recovery_codes` while it rotates the session making the change. With sessions deleted first that pair is a cycle, measured on PostgreSQL as a deadlock that kills one side: a 500 on the route, or a collapse the operator has to run again. `auth_tokens` stays ahead of `auth_factors` because answering a challenge claims the challenge before it locks the factor, so the order is `auth_tokens`, `auth_factors`, `recovery_codes`, `sessions`, `auth_identities`. No foreign key ties those five together, so the order is free to choose. It closes the pairings the rotation opened rather than every pairing: a credential change and a collapse can still deadlock on `auth_identities`, as they could before this change, and no reordering of the deletes alone reaches that one, because the factor routes need sessions after `auth_factors` while the credential routes take `auth_identities` before sessions and `auth_tokens` after them.
