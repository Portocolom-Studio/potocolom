...
> Shipped status (2026-08-19): merged in #316. Only `vega-rt` names a `preview_decoder` in its manifest today. On the reference RX 7600 XT the shipped path measures 219.8 ms p95 against 336.4 ms p95 with the full VAE. An earlier revision of this entry claimed three slots from a 184.9 ms frame; that was wrong arithmetic and is corrected below.
...
First run therefore generates its secrets. The preflight script writes `deploy/compose/.env` when none exists, with `openssl rand -hex 32` for `FLEET_SECRET` and the database password, refuses to overwrite a file that is already there, and prints what it wrote. Hex because Compose expands `$NAME` inside an unquoted value and an HTTP header carries ASCII only, both of which a generated secret has to survive.
