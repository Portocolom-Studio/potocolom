...
`auth-clear-factor` is for somebody who turned on a second factor and then lost
both the authenticator and every recovery code. Anyone who still holds either
takes it off themselves from Security settings; this is the case where nothing
over HTTP can help, because a route that removes a factor without asking for
one is a route worth stealing a session for. It removes the factor and its
codes and touches nothing else: the account keeps its password, its role and
its sessions, and can enrol again whenever it likes.
