...
Browser to API: `{"type": "open", "model_id": "sd-sim", "params": {"prompt": "a red house"}}` first, then binary canvas frames carrying the session id, then `{"type": "close"}`.

API to browser: `{"type": "ready", "session_id": "..."}`, generated frames as binary, and during recovery `{"type": "interrupted"}` then `{"type": "resumed"}` (re-send the current canvas). Terminal failures arrive as `error` messages before the close. A worker that cannot make the requested model fully resident refuses the session, and the browser sees the refusal code 4003 in both cases: at open, the `open` is answered with `{"type": "error", "code": 4003, "message": "worker did not become ready"}` before the close, and a live session whose model stops being fully resident ends with `{"type": "error", "code": 4003, "message": "worker could not serve the session"}` before the close. Both are the failure state, not the interrupted state: the model's residency on the assigned worker is what failed, so retrying the same open against that worker would be refused the same way. Issue #19 adds `queued` with a live position, `idle` and `resuming` for slot release, prompt updates, `credits_tick`, and an out of credits close (an `error` message then the close) when a session's chunked reservation cannot be extended.
