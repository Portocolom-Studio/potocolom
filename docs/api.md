...
- Removing a second factor costs a code, the same as adding one: recent authentication and either a current code or a recovery code. A session alone is not enough, for the reason enrolment writes nothing until a code answers, and the recovery codes go with the factor. An account that has lost the authenticator and every code has nothing to present, and no route can help it: that way back is `make auth-clear-factor` at the machine.
...
| DELETE `/api/v1/account/totp` | implemented | remove the second factor; needs recent authentication and a current code or a recovery code, and takes the recovery codes with it |
