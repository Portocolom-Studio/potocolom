...
API to browser: `{"type": "ready", "session_id": "..."}`, generated frames as binary, and during recovery `{"type": "interrupted"}` then `{"type": "resumed"}` (re-send the current canvas). An accepted parameter update is confirmed with `{"type": "params_updated", "params": {...}}` carrying what actually applied. Terminal failures arrive as `error` messages before the close. A rejected `update_params` (invalid params, a `seed` change - a session's seed is fixed at open - or an assigned worker whose protocol predates `update_session`) also arrives as an `error` but leaves the session running. A worker that cannot make the requested model fully resident refuses the session at open, and the browser sees the refusal code 4003: the `open` is answered with `{"type": "error", "code": 4003, "message": "worker did not become ready"}` before the close. This is the failure state, not the interrupted state: the model's residency on the assigned worker is what failed, so retrying the same open against that worker would be refused the same way. Issue #19 adds `queued` with a live position, `idle` and `resuming` for slot release, `credits_tick`, and an out of credits close (an `error` message then the close) when a session's chunked reservation cannot be extended.
