...
Canvas payloads larger than 1 MiB after the 17-byte header are dropped by the API and the realtime socket stays open. Uvicorn receives at most 2 MiB (`--ws-max-size`). A larger message closes the socket with 1009. The worker takes the same 2 MiB on its fleet socket, so the largest canvas frame the API forwards (17 bytes plus 1 MiB) reaches it.
