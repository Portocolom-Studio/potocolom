...
| `params_updated` | `params` | the merged set the worker actually applies; sent only when the update was forwarded |
...
| 4003 | no worker capacity for the requested model; also closes an open whose assigned worker refused it (the model could not be made fully resident) | browser |
