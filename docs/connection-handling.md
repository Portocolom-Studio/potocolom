...
`control_generation` is the realtime counterpart of `dispatch_token`, and it is a counter rather than a token because realtime needs ordering as well as identity. A token could tell two attempts apart but not say which came later, which is what a delayed `close_session` to a still-connected worker needs. It starts at 1 for a session's first attempt and increases by one for every attempt after it, travelling on every lifecycle control. It does not travel on frames: the binary header above is 17 bytes with no room for it, and issue #19 owns the revision field that would add one. Frames are fenced by the runner they reach instead. A canvas frame lands on whichever runner the worker holds for that session, and that runner is the fenced one because only a fenced open installs it; a generated frame is accepted only from the worker the session is assigned to. What closes the gap is the rule below that accepting an open cancels the runner it replaces, which leaves no unfenced source of frames alive. Cancelling a runner mid-frame is safe rather than lossy in the wrong direction: a cancelled runner cannot abandon the GPU thread it started, so the frame inside the lock runs to completion and is then discarded, because the send comes after the await that raises. Nothing stale escapes, and the cost is that the next attempt's first frame waits for that frame to leave the lock, which bounds how long a superseded attempt can hold up its replacement at one frame time. If #19 later adds a revision to the header, the generation can ride there too and frames become fenced directly rather than by inference.
