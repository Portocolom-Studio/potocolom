...
The real time target is 2 to 4 generated frames per second at 512 px, which an SD-Turbo or LCM class model delivers on an RTX 4090 class GPU. On the reference RX 7600 XT at 512 px, the shipped defaults measure `sdxl-turbo` at 278 ms per frame on its single default step without a preview decoder, and with `preview_decoder` `madebyollin/taesdxl` (issue #323) at 157.0 ms p50 and 160.2 p95, and `vega-rt` at 219.8 ms p95 on its four default steps with the shipped distilled preview decoder, against 336.4 ms p95 with the full VAE on the same path. Each worker advertises its real time slots in its registration, and the count is measured, not configured: at model warmup the worker times single frames on every remaining realtime model and sets scalar `realtime_slots` to the minimum of those slot counts (floor of the 500 ms budget over that model's p95), capped by the configured REALTIME_SLOTS. Current workers also send a top-level `realtime_p95_ms` map so the API admits by serialized frame cost rather than applying one model's count to every model; compatible frames now share one GPU cycle (issue #294), and admission still uses that serialized cost until a measured batch curve replaces it. Two serialised `vega-rt` sessions therefore cost about 440 ms per cycle, twice the measured single-frame p95, which is inside the 500 ms bar, so a worker serving that model advertises two slots without any scheduler change and concurrency above one session no longer waits on batching. Both realtime models now name `preview_decoder` `madebyollin/taesdxl`. Without that field the studio default measured 276.4 ms p95 and advertised one slot; with it, 160.2 ms and three. Calibration on the card is what advertises the live count. That 440 ms is arithmetic on a measured frame rather than a measured pair of sessions. Issue #294 targets the fourth session and cheaper cloud GPUs rather than the second. Two sessions rendering at once has not been tested end to end: until the preview decoder shipped, a worker advertised exactly one slot and a second session could not be opened. A worker cannot raise capacity on a live connection: heartbeat may raise a model's admission p95, which lowers new admissions, but must not lower it (the heartbeat half of issue #317). Ending a live session that no longer fits still needs `session_refused` (issue #270). The scheduler admits a session when live costs on that worker plus the new model's p95 stay inside 500 ms, never against hope: it does not oversubscribe. Worker-internal batching has shipped below the slot abstraction (issue #294). StreamDiffusion-class pipeline optimizations stay deferred with an explicit trigger in [decisions.md](decisions.md) ("GPU session density"). The scheduler still consumes slots and never sees batches.
