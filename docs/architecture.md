...
- **Pixel upscale** (`upscale` capability): Real-ESRGAN class weights, announced as their own manifest. `POST /api/v1/generations` with `source_asset_id` and `params.factor` routes here when the pinned model declares `upscale` and nothing else from `{text_to_image, image_to_image}`. Dispatch, retry, SSE progress, thumbnails, lineage (`parent_asset_id`), and `gpu_ms` metering all reuse the job path above.
