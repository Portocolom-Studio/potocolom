...
@pytest.mark.db
def test_an_account_route_answers_503_while_the_store_is_down(accounts):
...
@pytest.mark.db
def test_a_password_reset_closes_the_sockets_the_stolen_session_held(accounts):
...
@pytest.mark.db
def test_a_password_change_closes_every_other_socket_and_keeps_its_own(accounts):
    with TestClient(app, base_url=ORIGIN) as client:
    ...
    assert len(closed) == 1
    assert closed[0][0] == user.id
    assert closed[0][2] == principal.session.id


def _watch_closures() -> list:
