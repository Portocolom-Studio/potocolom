...
- **`AUTH_MODE=none` is the shipped default** and resolves every request to one implicit local admin. Setting `local` or `oauth` refuses to start.
...
- **Asset reads use opaque asset IDs.** The API checks the asset owner or admin role, returns 404 for missing or unauthorized assets, and returns 400 for an unsafe download name. The retired storage-key GET route always returns 404. Worker input remains capability-bound, with an opaque 32-byte capability that expires after 15 minutes and does not reveal the storage key.
