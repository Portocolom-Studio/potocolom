...
# /dev/kfd is the AMD compute device, absent on display-only amdgpu; NVIDIA is
# read from /proc/driver/nvidia because a laptop dGPU idles in D3cold with its
# modules unloaded, so lsmod is silent on a working install.
detect_worker() {
	if [[ -e /dev/kfd ]]; then
		echo rocm
